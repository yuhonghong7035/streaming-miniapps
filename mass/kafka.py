...
For light source, broker config (server.properties) needs to be adjusted to
...
def produce_block_kmeans_static(block_id=1,
    ...
    print "Zookeeper: %s, Block Id: %s, Num Cluster: %d" % (
        kafka_zk_hosts, str(block_id), NUMBER_CLUSTER)
    # init Kafka
...
def produce_block_kmeans(block_id=1,
    ...
    end_data_generation = time.time()
    print "Points Array Shape: %s, Number Batches: %.1f" % (points_np.shape,
                                                            number_messages)
...
def get_lightsource_data():
    ...
    with open(data_file, "r") as f:
        data = f.read()
    print("Access sample data: " + module + "; File: tooth.h5; Size: " +
          str(len(data)))
    return data
...
def produce_block_cms_data(block_id=1, kafka_zk_hosts=None,  number_messages=1,
                           topic_name=TOPIC_NAME):
    start = time.time()
...
def get_cms_data(directory=None):
    ...
    if directory is None:
        ...
        00000d3a.hd5; Size: " + str(len(data)))
    else:
        pass  # add data url?
...
class MiniApp():
    ...
    def __init__(
        ...
        self.number_total_points = self.number_points_per_cluster * \
            self.number_clusters
        self.number_dim = number_dim

        if self.application_type.startswith("kmeans"):
            self.number_messages = (self.number_points_per_cluster *
                                    self.number_clusters) / self.number_points_per_message
        elif self.application_type.startswith("light") or \
                self.application_type.startswith("BNL"):
        ...
        self.number_dask_cores_per_worker = dask_scheduler_info['workers'][dask_scheduler_info['workers'].keys()[
            0]]["ncores"]
    ...
    def clean_kafka(self):
        cmd = "%s/bin/kafka-topics.sh --delete --zookeeper %s --topic %s" % (
            KAFKA_HOME, self.kafka_zk_hosts, self.topic_name)
        print cmd
        # os.system(cmd)
        # time.sleep(60)
        ...
        cmd = "%s/bin/kafka-topics.sh --describe --zookeeper %s --topic %s" % (
            KAFKA_HOME, self.kafka_zk_hosts, self.topic_name)
        print cmd
        os.system(cmd)
        ...
        while count_produces < self.number_produces:
            ...
            for block_id in range(self.number_parallel_tasks):
                ...
                if self.application_type.startswith("kmeansstatic"):
                    ...
                    tasks.append(t)
                elif self.application_type.startswith('BNL'):
                    number_messages_per_task = self.number_messages/self.number_parallel_tasks
                    t = delayed(produce_block_cms_data, pure=False)(
                        block_id,
                        self.kafka_zk_hosts,
                        number_messages_per_task,
                        topic_name=self.topic_name
